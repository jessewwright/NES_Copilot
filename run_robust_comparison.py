...
# Add this specific import for CheckpointManager
...
def fit_nes_model(
    ...
    # Retain the existing mock InferenceData generation for now:
    try:
        logger.warning("Retaining OLD mock InferenceData generation for now. Actual samples are in posterior_samples_dict.")

        _num_samples_per_chain_mock = num_posterior_samples // num_chains
        if _num_samples_per_chain_mock == 0 and num_posterior_samples > 0:
            _num_samples_per_chain_mock = 1
        elif _num_samples_per_chain_mock == 0 and num_posterior_samples == 0:
            logger.error("Mock data generation: num_posterior_samples is 0. Cannot create mock idata.")
            return None

        _posterior_data_mock = {}
        for param in nes_param_names:
             _posterior_data_mock[param] = np.random.normal(0.5, 0.2, (num_chains, _num_samples_per_chain_mock))

        for param, value in fixed_nes_params.items():
            _posterior_data_mock[param] = np.full((num_chains, _num_samples_per_chain_mock), value)

        mock_idata = az.from_dict(
            posterior=_posterior_data_mock,
            coords={'chain': np.arange(num_chains), 'draw': np.arange(_num_samples_per_chain_mock)},
        )

        n_trials = len(subject_data) # n_trials for log likelihood
        n_trials_ll = max(n_trials, 10)
        log_lik_mock = np.random.normal(-1, 0.5, (num_chains, _num_samples_per_chain_mock, n_trials_ll))
        coords_loglik = {
            'chain': np.arange(num_chains),
            'draw': np.arange(_num_samples_per_chain_mock),
            'trial_ll': np.arange(n_trials_ll)
        }
        log_lik_ds_mock = xr.Dataset(
            data_vars={'nes': (['chain', 'draw', 'trial_ll'], log_lik_mock)},
            coords=coords_loglik
        )
        mock_idata.add_groups({'log_likelihood': log_lik_ds_mock})
        
        logger.info(f"Mock NES InferenceData object created (actual samples in posterior_samples_dict).")
        return mock_idata

    except Exception as e:
        logger.error(f"Error in mock fit_nes_model (after actual sampling attempt): {e}", exc_info=True)
        return None
